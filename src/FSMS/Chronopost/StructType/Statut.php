...
class Statut extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \FSMS\Chronopost\StructType\Entry[]
     */
    protected ?array $entry = null;
    /**
     * Constructor method for statut
     * @uses Statut::setEntry()
     * @param \FSMS\Chronopost\StructType\Entry[] $entry
     */
    ...
    /**
     * Get entry value
     * @return \FSMS\Chronopost\StructType\Entry[]
     */
    ...
    public static function validateEntryForArrayConstraintFromSetEntry(?array $values = []): string
    {
        ...
        foreach ($values as $statutEntryItem) {
            ...
            if (!$statutEntryItem instanceof \FSMS\Chronopost\StructType\Entry) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The entry property can only contain items of type \FSMS\Chronopost\StructType\Entry, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \FSMS\Chronopost\StructType\Entry[] $entry
     * @return \FSMS\Chronopost\StructType\Statut
     */
     ...
     * @throws InvalidArgumentException
     * @param \FSMS\Chronopost\StructType\Entry $item
     * @return \FSMS\Chronopost\StructType\Statut
     */
    public function addToEntry(\FSMS\Chronopost\StructType\Entry $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \FSMS\Chronopost\StructType\Entry) {
            throw new InvalidArgumentException(sprintf('The entry property can only contain items of type \FSMS\Chronopost\StructType\Entry, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        $this->entry[] = $item;
