...
class DestinatairesDpd extends AbstractStructBase
{
    ...
    public static function validateDestinataireDpdForArrayConstraintFromSetDestinataireDpd(?array $values = []): string
    {
        ...
        foreach ($values as $destinatairesDpdDestinataireDpdItem) {
            ...
            if (!$destinatairesDpdDestinataireDpdItem instanceof \FSMS\Chronopost\StructType\DestinataireDpd) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The destinataireDpd property can only contain items of type \FSMS\Chronopost\StructType\DestinataireDpd, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \FSMS\Chronopost\StructType\DestinataireDpd[] $destinataireDpd
     * @return \FSMS\Chronopost\StructType\DestinatairesDpd
     */
     ...
     * @throws InvalidArgumentException
     * @param \FSMS\Chronopost\StructType\DestinataireDpd $item
     * @return \FSMS\Chronopost\StructType\DestinatairesDpd
     */
    public function addToDestinataireDpd(\FSMS\Chronopost\StructType\DestinataireDpd $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \FSMS\Chronopost\StructType\DestinataireDpd) {
            throw new InvalidArgumentException(sprintf('The destinataireDpd property can only contain items of type \FSMS\Chronopost\StructType\DestinataireDpd, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
