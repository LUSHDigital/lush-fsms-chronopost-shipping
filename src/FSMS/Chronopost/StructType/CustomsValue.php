...
class CustomsValue extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \FSMS\Chronopost\StructType\ArticleValue[]
     */
     ...
     * @param \FSMS\Chronopost\StructType\ArticleValue[] $articlesValue
     */
     ...
     * @return \FSMS\Chronopost\StructType\ArticleValue[]
     */
    ...
    public static function validateArticlesValueForArrayConstraintFromSetArticlesValue(?array $values = []): string
    {
        ...
        foreach ($values as $customsValueArticlesValueItem) {
            ...
            if (!$customsValueArticlesValueItem instanceof \FSMS\Chronopost\StructType\ArticleValue) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The articlesValue property can only contain items of type \FSMS\Chronopost\StructType\ArticleValue, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \FSMS\Chronopost\StructType\ArticleValue $item
     * @return \FSMS\Chronopost\StructType\CustomsValue
     */
    public function addToArticlesValue(\FSMS\Chronopost\StructType\ArticleValue $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \FSMS\Chronopost\StructType\ArticleValue) {
            throw new InvalidArgumentException(sprintf('The articlesValue property can only contain items of type \FSMS\Chronopost\StructType\ArticleValue, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
