...
class ResultMultiParcelExpeditionValue extends ResultShippingValue
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \FSMS\Chronopost\StructType\ResultMultiParcelValue[]
     */
     ...
     * @param \FSMS\Chronopost\StructType\ResultMultiParcelValue[] $resultMultiParcelValue
     */
     ...
     * @param string $eSDFullNumber
     * @return \FSMS\Chronopost\StructType\ResultMultiParcelExpeditionValue
     */
     ...
     * @return \FSMS\Chronopost\StructType\ResultMultiParcelValue[]
     */
    ...
    public static function validateResultMultiParcelValueForArrayConstraintFromSetResultMultiParcelValue(?array $values = []): string
    {
        ...
        foreach ($values as $resultMultiParcelExpeditionValueResultMultiParcelValueItem) {
            ...
            if (!$resultMultiParcelExpeditionValueResultMultiParcelValueItem instanceof \FSMS\Chronopost\StructType\ResultMultiParcelValue) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The resultMultiParcelValue property can only contain items of type \FSMS\Chronopost\StructType\ResultMultiParcelValue, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \FSMS\Chronopost\StructType\ResultMultiParcelValue $item
     * @return \FSMS\Chronopost\StructType\ResultMultiParcelExpeditionValue
     */
    public function addToResultMultiParcelValue(\FSMS\Chronopost\StructType\ResultMultiParcelValue $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \FSMS\Chronopost\StructType\ResultMultiParcelValue) {
            throw new InvalidArgumentException(sprintf('The resultMultiParcelValue property can only contain items of type \FSMS\Chronopost\StructType\ResultMultiParcelValue, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
