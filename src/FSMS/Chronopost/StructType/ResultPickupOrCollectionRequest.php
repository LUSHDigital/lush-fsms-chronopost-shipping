...
class ResultPickupOrCollectionRequest extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \FSMS\Chronopost\StructType\InfoEnlevement[]
     */
     ...
     * @param \FSMS\Chronopost\StructType\InfoEnlevement[] $infoEnlevements
     */
     ...
     * @param int $codeErreur
     * @return \FSMS\Chronopost\StructType\ResultPickupOrCollectionRequest
     */
     ...
     * @return \FSMS\Chronopost\StructType\InfoEnlevement[]
     */
    ...
    public static function validateInfoEnlevementsForArrayConstraintFromSetInfoEnlevements(?array $values = []): string
    {
        ...
        foreach ($values as $resultPickupOrCollectionRequestInfoEnlevementsItem) {
            ...
            if (!$resultPickupOrCollectionRequestInfoEnlevementsItem instanceof \FSMS\Chronopost\StructType\InfoEnlevement) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The infoEnlevements property can only contain items of type \FSMS\Chronopost\StructType\InfoEnlevement, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \FSMS\Chronopost\StructType\InfoEnlevement $item
     * @return \FSMS\Chronopost\StructType\ResultPickupOrCollectionRequest
     */
    public function addToInfoEnlevements(\FSMS\Chronopost\StructType\InfoEnlevement $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \FSMS\Chronopost\StructType\InfoEnlevement) {
            throw new InvalidArgumentException(sprintf('The infoEnlevements property can only contain items of type \FSMS\Chronopost\StructType\InfoEnlevement, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
