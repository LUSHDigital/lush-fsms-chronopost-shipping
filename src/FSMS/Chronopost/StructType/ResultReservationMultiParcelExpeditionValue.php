...
class ResultReservationMultiParcelExpeditionValue extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - nillable: true
     * @var \FSMS\Chronopost\StructType\ResultParcelValue[]
     */
     ...
     * @param \FSMS\Chronopost\StructType\ResultParcelValue[] $resultParcelValue
     */
     ...
     * @param string $eSDFullNumber
     * @return \FSMS\Chronopost\StructType\ResultReservationMultiParcelExpeditionValue
     */
     ...
     * @return \FSMS\Chronopost\StructType\ResultParcelValue[]
     */
    ...
    public static function validateResultParcelValueForArrayConstraintFromSetResultParcelValue(?array $values = []): string
    {
        ...
        foreach ($values as $resultReservationMultiParcelExpeditionValueResultParcelValueItem) {
            ...
            if (!$resultReservationMultiParcelExpeditionValueResultParcelValueItem instanceof \FSMS\Chronopost\StructType\ResultParcelValue) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The resultParcelValue property can only contain items of type \FSMS\Chronopost\StructType\ResultParcelValue, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \FSMS\Chronopost\StructType\ResultParcelValue $item
     * @return \FSMS\Chronopost\StructType\ResultReservationMultiParcelExpeditionValue
     */
    public function addToResultParcelValue(\FSMS\Chronopost\StructType\ResultParcelValue $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \FSMS\Chronopost\StructType\ResultParcelValue) {
            throw new InvalidArgumentException(sprintf('The resultParcelValue property can only contain items of type \FSMS\Chronopost\StructType\ResultParcelValue, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
